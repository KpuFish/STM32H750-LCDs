...
void LCD_Putchar(uint16_t x,uint16_t y,uint8_t num,uint8_t mode)
{
  ...
  if(x > (ILI9341_GetWidth() - 8) || y > ILI9341_GetHeight() - 16) return;
  num=num-' ';
  ...
  if(!mode)
  {
    for(pos = 0; pos < 16; pos++)
    {
      ...
      for(t = 0; t < 8; t++)
      {
        if(temp&0x01)ILI9341_Draw_Pixel(x+t,y+pos,POINT_COLOR);
        else ILI9341_Draw_Pixel(x+t,y+pos,BACK_COLOR);
        temp>>=1;
      }
    }
  }
...
{
  if((x>ILI9341_GetWidth()-14)||(y>ILI9341_GetHeight()-24)) return;
  uint8_t i,j,k,temp;
    ...
    for(j=0;j<8;j++)
    {
      for(k=0;k<14;k++)
      {
        ...
        if(mode==TFT_STRING_MODE_BACKGROUND)
        {
          if(temp&(0x01<<j))
          {
          	ILI9341_Draw_Pixel(x+k,y+(8*i+j),POINT_COLOR);
          }
          else
          {
          	ILI9341_Draw_Pixel(x+k,y+(8*i+j),BACK_COLOR);
          }
        }
        else
        {
...
{
  if((x>ILI9341_GetWidth()-18)||(y>ILI9341_GetHeight()-32)) return;
  uint8_t i,j,k,temp;
...
{
  if((x>ILI9341_GetWidth()-26)||(y>ILI9341_GetHeight()-48)) return;
  uint8_t i,j,k,temp;
...
void LCD_Puts8x16(uint16_t x, uint16_t y, uint8_t *string, uint8_t TFT_STRING_MODE)
{
  ...
  while(*(string+i)!='\0')
  {
    ...
    if(x > ILI9341_GetWidth() - font_w)
    {
      x = 0;
      y += font_h;
    }

    if(y > ILI9341_GetHeight() - font_h)
    {
      x = y = 0;
    }
